...
const MAX_PER_STOCK = 2000; // Hard cap per stock position in dollars
...
let stockTimer = null, cryptoTimer = null;
let lastResetDay = -1;
...
// ─── DAILY RESET ─────────────────────────────────────────────────────────────
// Resets counters and daily P&L at market open each day (9:30am ET = 13:30 UTC)
function maybeDailyReset() {
  const now   = new Date();
  const day   = now.getUTCDay();   // 0=Sun, 6=Sat
  const hour  = now.getUTCHours();
  const min   = now.getUTCMinutes();
  const todayNum = now.toISOString().slice(0, 10); // "2025-05-05"

  // Only reset on weekdays at or after 13:30 UTC
...
// ─── SIGNAL ENGINE ────────────────────────────────────────────────────────────
// Requires 10 price samples. Signal fires at 0.15% deviation from MA10.
function getSig(h) {
...
function addPx(hist, sym, price) {
  ...
  const arr = hist[sym];
  // Always push; deduplicate consecutive identical prices
...
function isMarketHours() {
  ...
  // 9:30am–4:00pm ET (UTC-4 summer / UTC-5 winter); use UTC 13:30–20:00 as safe window
...
async function stockTick() {
  ...
  if (!isMarketHours()) {
    // Outside market hours — just collect prices if available, skip trading
    return;
  }
  ...
  try {
    ...
    // Reset counters for symbols we no longer hold
    ...
    for (const sym of STOCKS) {
      ...
      if (posMap[sym]) {
        ...
        continue; // Don't try to buy if we already hold
      }
      ...
      if (sig.type === "BUY" && sig.confidence >= 65) {
        ...
        if (room <= 50) continue; // Less than $50 room, skip

        // Divide remaining room evenly across planned entries
  ...
  try {
    ...
    for (const sym of CRYPTO) {
      ...
      if (sig.type === "BUY" && sig.confidence >= 75) {
        ...
        const budget = (usd * 0.10) / entriesLeft; // 10% of USD per entry slice
        if (budget < 1 || usd < 5) continue;

        const coin    = sym.replace("-USD", "");
        const holding = parseFloat(cbAcc[coin]?.available_balance?.value || 0);
        if (holding * price > MAX_PER_STOCK) continue; // $2k crypto cap too
...
app.get("/prices", (req, res) => {
  res.json({ prices: { ...sPrices, ...cPrices } });
});
