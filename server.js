...
console.log("[BOOT] CB key name:", CB_KEY_NAME ? CB_KEY_NAME.substring(0, 40) + "..." : "MISS
console.log("[BOOT] CB private key:", CB_PRIVATE_KEY ? CB_PRIVATE_KEY.substring(0, 27) + "...
// ─── UNIVERSE ─────────────────────────────────────────────────────────────────
const STOCKS = [
...
const header = Buffer.from(JSON.stringify({ alg: "ES256", kid: CB_KEY_NAME, nonce })).toS
...
if (sTrades.length > 200) sTrades.pop();
console.log(`[BUY] ${sym} ${qty}sh @ $${price} | Entry ${sEntryCount[sym]}/${maxEnt
}
}
}
} catch (e) {
...
console.warn("[CB price]", pair, "unexpected response:", JSON.stringify(res).substr
}
} catch (e) {
...
if (period === "month") { const s = new Date(); s.setDate(1); s.setHours(0,0,0,0); return true;
return
});
...
const qty = Math.abs(parseInt(pos.qty));
const ord = await apost("/v2/orders", { symbol: sym, qty, side: "sell", type: "market",
if (ord.id) {
...
const qty = Math.abs(parseInt(pos.qty));
const ord = await apost("/v2/orders", { symbol: sym, qty, side: "sell", type: "market", t
if (ord.id) {
const pv = parseFloat(pos.unrealized_pl || 0);
sPnl += pv;
sTrades.unshift({
...
app.listen(PORT, () => {
console.log(`[APEX TRADE] Listening on port ${PORT} | PAPER=${PAPER} | MAX_PER_STOCK=$${MAX
stockRunning = true;
stockTimer = setInterval(stockTick, 10000);
stockTick();
cryptoRunning = true;
